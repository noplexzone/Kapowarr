...
function loadWorker(scope = 'https://example.test/kapowarr/') {
  ...
  vm.runInNewContext(`${source}\n;globalThis.__test = { CACHE_NAME, shouldCacheRequest };`, context);
...
describe('Kapowarr service worker cache boundaries', () => {
  ...
  it('allows only same-origin shell and static frontend assets', () => {
    const { api } = loadWorker();
    expect(api.shouldCacheRequest(new Request('https://example.test/kapowarr/'))).toBe(true);
    expect(api.shouldCacheRequest(new Request('https://example.test/kapowarr/manifest.json'))).toBe(true);
  ...
  it('versions its cache and deletes only stale Kapowarr cache namespaces', async () => {
    ...
    await activation;
    expect(api.CACHE_NAME).toMatch(/^kapowarr-static-v\d+$/);
